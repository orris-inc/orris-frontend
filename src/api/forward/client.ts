...
import type {
  ...
  ForwardChain,
  CreateForwardChainRequest,
  UpdateForwardChainStatusRequest,
  ListForwardChainsParams,
} from './types';
...
/**
 * Create forward chain (Admin only)
 * POST /forward-chains
 */
export const createForwardChain = async (
  data: CreateForwardChainRequest
): Promise<ForwardChain> => {
  const response = await apiClient.post<APIResponse<ForwardChain>>('/forward-chains', data);
  return response.data.data;
};

/**
 * Get forward chain by ID (Admin only)
 * GET /forward-chains/:id
 */
export const getForwardChain = async (id: number | string): Promise<ForwardChain> => {
  const response = await apiClient.get<APIResponse<ForwardChain>>(`/forward-chains/${id}`);
  return response.data.data;
};

/**
 * List forward chains (Admin only)
 * GET /forward-chains
 */
export const listForwardChains = async (
  params?: ListForwardChainsParams
): Promise<ListResponse<ForwardChain>> => {
  const response = await apiClient.get<APIResponse<ListResponse<ForwardChain>>>(
    '/forward-chains',
    { params }
  );
  return response.data.data;
...
/**
 * Delete forward chain (Admin only)
 * DELETE /forward-chains/:id
 */
export const deleteForwardChain = async (id: number | string): Promise<void> => {
  await apiClient.delete(`/forward-chains/${id}`);
};

/**
 * Update forward chain status (Admin only)
 * PATCH /forward-chains/:id/status
 */
export const updateForwardChainStatus = async (
  id: number | string,
  data: UpdateForwardChainStatusRequest
): Promise<void> => {
  await apiClient.patch(`/forward-chains/${id}/status`, data);
};

/**
 * Enable forward chain (Admin only)
 * POST /forward-chains/:id/enable
 */
export const enableForwardChain = async (id: number | string): Promise<void> => {
  await apiClient.post(`/forward-chains/${id}/enable`);
};

/**
 * Disable forward chain (Admin only)
 * POST /forward-chains/:id/disable
 */
export const disableForwardChain = async (id: number | string): Promise<void> => {
  await apiClient.post(`/forward-chains/${id}/disable`);
};
