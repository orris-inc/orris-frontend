 ...
 *
 * Recent changes:
 * - 2026-08-12: sortOrder is now one sequence shared with direct nodes. reorderForwardRules (admin)
 *   writes absolute positions verbatim; reorderUserForwardRules and reorderSubscriptionForwardRules
 *   read the values as relative order only. New rules are appended to the end instead of defaulting
 *   to 0. To position rules against direct nodes, use the node module's subscription order APIs.
 ...
 *
 * IMPORTANT (2026-08-12): sortOrder is now one sequence shared with direct nodes, and
 * admin values are written through verbatim. Sending dense 1..N indices will therefore
 * push these rules ahead of every direct node in the subscription. Send absolute
 * positions read from the current order instead. To position rules against direct nodes,
 * prefer the node module's getSubscriptionOrder / reorderSubscriptionOrder, which lists
 * both kinds together.
 *
 ...
 *
 * Added: 2025-12-23
 * Updated: 2026-08-12 - sortOrder now shares one sequence with direct nodes
 */
 ...
 *
 * Since 2026-08-12 only the *relative* order of the submitted sortOrder values matters
 * here. The rules keep the positions they already occupy in the subscription and merely
 * swap places among themselves, so dense 1..N indices from a drag-and-drop list are safe
 * and cannot move these rules ahead of the direct nodes. The stored values will differ
 * from what you send — re-read the list rather than assuming it echoes back.
 *
 * @param data - Reorder request containing array of rule orders
 *
 * @example
 ...
 *
 * Added: 2025-12-23
 * Updated: 2026-08-12 - values are interpreted as relative order, not absolute positions
 */
