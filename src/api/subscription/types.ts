 ...
 *
 * Recent changes:
 * - 2026-04-29: Added currentTrafficCycleStart/End, currentCycleUploadBytes/DownloadBytes
 *   to Subscription. The traffic cycle window is the authoritative window for
 *   data usage figures (calendar_month or billing_cycle, depending on plan).
 *   It differs from currentPeriodStart/End for calendar_month plans.
 * - 2026-02-28: Added dataUsedBytes, dataLimitBytes to Subscription and AdminSubscriptionDTO
 * - 2026-02-05: Added billingCycle to Subscription (synced with backend SubscriptionDTO)
 * - 2026-01-30: Added nodeLimit to SubscriptionPlan, CreatePlanRequest, UpdatePlanRequest (synced with backend)
 * - 2026-01-29: Removed apiRateLimit, maxUsers, maxProjects from SubscriptionPlan, CreatePlanRequest, UpdatePlanRequest
 * - 2025-01-14: Updated SubscriptionStatus to include all 8 backend statuses (added: inactive, pending_payment, trialing, past_due, suspended)
...
/**
 * Subscription entity
 * Updated: 2026-04-29 - Added currentTrafficCycleStart/End and currentCycleUpload/DownloadBytes
 * Updated: 2026-02-05 - Added billingCycle field
...
export interface Subscription {
  ...
  /** Billing cycle: weekly, monthly, quarterly, semi_annual, yearly, lifetime */
  billingCycle?: BillingCycle;
  startDate: string;
  endDate: string;
  autoRenew: boolean;
  /** Subscription billing period start (used by billing/renewal flows). */
  currentPeriodStart: string;
  /** Subscription billing period end. */
  currentPeriodEnd: string;
  /**
   * Current traffic cycle start. Pair with dataUsedBytes / currentCycleUpload(Download)Bytes.
   * For calendar_month plans this is the business-timezone calendar month start
   * and differs from currentPeriodStart; for billing_cycle plans it matches.
   * Added: 2026-04-29.
   */
...
/**
 * Subscription status
 * Updated: 2025-01-14 - Synced with backend status.go, added all 8 statuses
 */
export type SubscriptionStatus =
  | 'inactive'        // Initial state, not yet activated
  | 'pending_payment' // Waiting for payment
  | 'trialing'        // In trial period
  | 'active'          // Currently active
  | 'past_due'        // Payment overdue but still accessible
  | 'suspended'       // Suspended due to traffic limit exceeded or other reasons
  | 'cancelled'       // User cancelled
  | 'expired';        // Subscription expired

/**
 * Subscription plan entity
 * Matches backend PlanDTO structure from internal/application/subscription/dto/dto.go
 *
 * Updated: 2026-01-30 - Synced with backend PlanDTO (added nodeLimit, removed trialDays)
...
export interface SubscriptionPlan {
  ...
  planType: PlanType;
  /** Plan limits configuration (traffic_limit, etc.) */
  limits?: Record<string, unknown>;
  /** Maximum number of user-created nodes (null or 0 = unlimited) */
  nodeLimit?: number;
  isPublic: boolean;
  sortOrder: number;
 ...
 * Create plan request
 * POST /plans
 * Matches backend CreatePlanRequest in planhandler.go
 *
 * Updated: 2026-01-30 - Synced with backend (added nodeLimit, removed trialDays)
 */
export interface CreatePlanRequest {
  ...
  planType: PlanType;
  /** Plan limits configuration (e.g., traffic_limit) */
  limits?: Record<string, unknown>;
  /** Maximum number of user-created nodes (null or 0 = unlimited) */
  nodeLimit?: number;
  isPublic?: boolean;
 ...
 * Update plan request
 * PUT /plans/:id
 * Matches backend UpdatePlanRequest in planhandler.go
 *
 * Updated: 2026-01-30 - Synced with backend (added nodeLimit)
 */
 ...
 * Updated: 2025-12-23 - Changed nodeId to resourceType + resourceId (SID format)
 */
export interface TrafficStatsRecord {
  /** Resource type: "node" or "forward_rule" */
  resourceType: string;
  /** Resource's Stripe-style ID (node_xxx or fwd_xxx) */
  resourceId: string;
  upload: number;
  download: number;
  total: number;
