 ...
 *
 * Recent changes:
 * - 2026-08-12: Added getSubscriptionOrder and reorderSubscriptionOrder for positioning direct
 *   nodes and forward rules within one subscription sequence. Subscriptions no longer render
 *   direct nodes as a block followed by forwarded ones — both are merged on a shared sort_order,
 *   so a direct node can sit between forwarded ones.
...
import type {
  ...
  GetSubscriptionOrderParams,
  ReorderSubscriptionOrderRequest,
} from './types';
 ...
 * @example
 * ```typescript
 * const result = await getBatchInstallScript({
 *   nodeIds: ['node_xxx', 'node_yyy', 'node_zzz']
 * });
 * console.log(result.installCommand);
 * // curl -fsSL ... | sudo bash -s -- --api-url https://api.example.com --node node_xxx:token1 --node node_yyy:token2 --node node_zzz:token3
 * ```
 */
export async function getBatchInstallScript(
  data: BatchInstallScriptRequest,
  apiUrl?: string
): Promise<BatchInstallScriptResponse> {
  const params = apiUrl ? { api_url: apiUrl } : undefined;
  const response = await apiClient.post<APIResponse<BatchInstallScriptResponse>>(
    '/nodes/batch-install-script',
    data,
    { params }
  );
  return response.data.data;
 ...
 * @example
 * ```typescript
 * const result = await getUserBatchInstallScript({
 *   nodeIds: ['node_xxx', 'node_yyy']
 * });
 * console.log(result.installCommand);
 * // curl -fsSL ... | sudo bash -s -- --api-url https://api.example.com --node node_xxx:token1 --node node_yyy:token2
 * ```
 */
export async function getUserBatchInstallScript(
  data: BatchInstallScriptRequest,
  apiUrl?: string
): Promise<BatchInstallScriptResponse> {
  const params = apiUrl ? { api_url: apiUrl } : undefined;
  const response = await apiClient.post<APIResponse<BatchInstallScriptResponse>>(
    '/user/nodes/batch-install-script',
    data,
    { params }
  );
  return response.data.data;
 ...
 *
 * @param nodeId - Stripe-style node ID (e.g., "node_xK9mP2vL3nQ")
...
// ============================================================================
// Subscription Order APIs (Admin)
// Added: 2026-08-12
// ============================================================================

/**
 * Get a resource group's subscription ordering (Admin only)
 * GET /nodes/subscription-order?group_id=rg_xxx
 *
 * Returns the group's origin nodes and the system forward rules bound to it as one
 * list, in the exact order a subscription renders them. Both kinds share a single
 * sort_order sequence, so an origin node can be positioned between forwarded ones.
 *
 * User-owned forward rules are not included: they belong to individual users rather
 * than to a resource group, and users position them via the forward rule reorder APIs.
 *
 * @param params - Must include the resource group ID
 * @requires Admin role
 *
 * @example
 * ```typescript
 * const items = await getSubscriptionOrder({ groupId: 'rg_xK9mP2vL3nQ' });
 * // [
 * //   { type: 'forward', id: 'fr_aaa',   name: 'HK relay', sortOrder: 100 },
 * //   { type: 'origin',  id: 'node_bbb', name: 'JP direct', sortOrder: 200 },
 * //   { type: 'forward', id: 'fr_ccc',   name: 'SG relay', sortOrder: 300 }
 * // ]
 * ```
 *
 * Added: 2026-08-12
...
export async function getSubscriptionOrder(
  params: GetSubscriptionOrderParams
): Promise<SubscriptionOrderItem[]> {
  const response = await apiClient.get<APIResponse<SubscriptionOrderItem[]>>(
    '/nodes/subscription-order',
    { params }
  );
  return response.data.data;
}
...
/**
 * Reposition entries in a subscription's ordering (Admin only)
 * PATCH /nodes/subscription-order
 *
 * Writes new positions for origin nodes and system forward rules in one transaction,
 * so a subscription never renders a half-applied order.
 *
 * Send absolute positions from the shared sequence, not dense 1..N indices: the values
 * are compared against every other entry, so `sortOrder: 1` puts an entry ahead of the
 * whole list. Read the current order first, then place entries between existing
 * neighbours. Positions are spaced by 100 to leave room for insertions.
 *
 * @param data - Entries to reposition
 * @requires Admin role
 *
 * @example
 * ```typescript
 * // Move a direct node in between two forwarded ones
 * const items = await getSubscriptionOrder({ groupId: 'rg_xK9mP2vL3nQ' });
 * // fr_aaa is at 100, fr_ccc is at 300 -> land the node at 200
 * await reorderSubscriptionOrder({
 *   items: [{ type: 'origin', id: 'node_bbb', sortOrder: 200 }]
 * });
 * ```
 *
 * Added: 2026-08-12
 */
export async function reorderSubscriptionOrder(
  data: ReorderSubscriptionOrderRequest
): Promise<void> {
  await apiClient.patch('/nodes/subscription-order', data);
}
