/**
 * Batch update nodes dialog
 * Allows admin to trigger updates for all nodes with available updates
 */
...
export const BatchUpdateDialog: React.FC<BatchUpdateDialogProps> = ({
  ...
  const nodesWithUpdates = nodes.filter((node) => node.hasUpdate && node.isOnline);
  const updateCount = nodesWithUpdates.length;
