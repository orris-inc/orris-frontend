...
export const SubscriptionForwardUsageCard: React.FC<SubscriptionForwardUsageCardProps> = ({
  ...
  return (
    <div className="p-5 rounded-xl bg-card border">
      <div className="space-y-4">
        <div>
          <h3 className="text-base font-medium text-foreground">转发配额</h3>
          <p className="text-sm text-muted-foreground">此订阅的转发规则配额和流量使用状态</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-lg bg-muted/50">
            ...
            <div className="space-y-1">
              <div className="flex items-baseline gap-1">
                <span className="text-xl font-semibold font-mono">{trafficUsed.value}</span>
                <span className="text-sm text-muted-foreground">{trafficUsed.unit}</span>
                {trafficLimit && (
                  <>
                    <span className="text-sm text-muted-foreground">
                      / {trafficLimit.value} {trafficLimit.unit}
                    </span>
                  </>
                )}
              </div>
