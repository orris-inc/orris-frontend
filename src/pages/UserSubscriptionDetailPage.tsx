...
export const UserSubscriptionDetailPage = () => {
  ...
  const backAction = (
    <Link
      ...
      className={cn(
        ...
        'h-8 px-2 gap-1.5 !min-w-0 sm:h-9 sm:px-3 sm:gap-2'
      )}
      aria-label={t('userSubscription.backToHome')}
    >
      <ArrowLeft className="size-4" />
      <span className="hidden sm:inline">{t('userSubscription.backToHome')}</span>
      <span className="sr-only sm:hidden">{t('userSubscription.backToHome')}</span>
    </Link>
  );
  ...
  if (isLoading) {
    return (
      <DashboardLayout
        pageTitle={t('userSubscription.details')}
        pageDescription={t('common.table.loading')}
        pageActions={backAction}
  ...
  if (error || !subscription) {
    return (
      <DashboardLayout
        pageTitle={t('userSubscription.details')}
        pageDescription={t('userSubscription.loadingFailed')}
        pageActions={backAction}
        inlineActionsOnMobile
      >
        <div className="space-y-4 md:space-y-6 animate-in fade-in duration-300 pb-safe">
          ...
          <div className="flex flex-col items-center justify-center py-12 md:py-16 px-4 glass-elevated rounded-2xl">
            ...
            <Button asChild className="w-full sm:w-auto min-h-11 glass-interactive hidden sm:inline-flex">
  ...
  return (
    <DashboardLayout
      pageTitle={subscription.plan?.name || t('userSubscription.details')}
      pageActions={backAction}
      inlineActionsOnMobile
    >
